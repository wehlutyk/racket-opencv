...
(define opencv-imgproc-lib
  (case (system-type)
    [(windows)
     (ffi-lib
      (build-path (system-library-subpath #f)
                  "libopencv_imgproc246"))]
    [(macosx) (ffi-lib "/opt/local/lib/libopencv_imgproc")]
    [else (ffi-lib "libopencv_imgproc")]))

(define-ffi-definer define-opencv-imgproc-internal opencv-imgproc-lib)

(define-syntax define-opencv-imgproc
  (syntax-rules ()
    [(_ name body)
     (begin
       (provide name)
       (define-opencv-imgproc-internal name body))]))
...
(define-opencv-imgproc cvRectangle
...
(define-opencv-imgproc cvInitFont
...
(define-opencv-imgproc cvCircle
...
(define-opencv-imgproc cvEllipse
...
(define-opencv-imgproc cvPutText
  (_fun _pointer _string _CvPoint _pointer _CvScalar
        -> _void))


#| Draws contour outlines or filled interiors on the image |#
(define-opencv-imgproc cvDrawContours
